...
namespace SaphiraTerror.Repositories
{
    public class FilmeRepository : IFilmeRepository
    {
        ...
        {
            return await _context.Filmes.Include(f => f.Genero).Include(f => f.Classificacao).Where(f => f.GeneroId == idGenero).ToListAsync();
        }

        // public async Task<Filme> GetByIdTituloAsync(int idTitulo)
        //{
